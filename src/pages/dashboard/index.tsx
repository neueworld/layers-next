...
const Main = () => {
  ...
  const { data: contracts, isLoading: loadingData } = useGetAllContractsByAddressQuery(
    { address, status: "all" },
    { skip }
  );
              ...
              <TabPanel px='0' pt='20px'>
                <Wrap w='full' spacing='30px'>
                  ...
                  {contracts?.map((contract: IContract) => {
                    return (
                      <WrapItem key={contract.slug} w={{ base: "full", xl: "initial" }}>
                        <DashboardCard
                          tag1={contract.category[0] as string}
                          tag2={contract.category[1] as string}
                          contractStatus={contract.status}
                          isAuthor={user?.address === contract.author.walletAddress}
                          author={contract.author}
                          guest={contract.guest}
                          address={truncateAddress(contract.guest.walletAddress)}
                          created={new Date(contract?.createdAt as string).toDateString()}
